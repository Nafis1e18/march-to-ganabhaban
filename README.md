...
raylib in the mould of *Cadillacs and Dinosaurs* — the game everyone in
Bangladesh remembers as **Mustafa**.

Five stages. Five bosses. Three lives. It gets harder every stage.

Rendered at **384×224**, the real CPS-1 arcade resolution the original ran on.
...
That's it — raylib is vendored in `vendor/raylib`, already compiled against
your toolchain. No install step.

| Action | Keyboard | Touch |
|---|---|---|
| Move (8 directions) | Arrows / WASD | left thumb pad |
| Punch — three-hit combo | `J` or `Z` | **HIT** |
...
at a similar depth — that single rule is what makes this a brawler rather than
...
clear. Then **GO →** appears.

Punch three times in a row for jab → cross → kick; the third hit knocks down.
...
fires. That telegraph is your cue — jump it, or change lane.
...
Difficulty climbs through `hpMul`, `dmgMul`, `spdMul` and — most importantly —
...
- **[src/stages.h](src/stages.h)** — enemy stats, wave composition, stage
  difficulty, boss assignments, scoring.
- **[src/game.h](src/game.h)** — frame data for the player's moves, the belt
...
art, the animation table and the hitboxes are three separate things — exactly
...
white or grey on the character itself** (it would be keyed out — tint light
...
| `--fast` | uncaps the framerate — a full 5-stage run takes seconds |
...
A verified full run looks like this — five stages, five bosses, victory:
...
src/audio.cpp     synthesised SFX and music — no sound files on disk
tools/            sprite extraction, background prep, drawing templates
assets/           what ships: packed sheets + background layers (~1.5 MB)
art_source/       the originals and templates — never shipped
```
